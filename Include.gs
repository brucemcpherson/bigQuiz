...
var Include = (function (ns) {
  ...
  */
  ...
  /**
  * given an array of .gs file names, and a lib it will get the source and return them concatenated for insertion into htmlservice
  * like this you can share the same code between client and server side, and use the Apps Script IDE to manage your js code
  * showMyScriptAppResource must exist in the library
  * @param {string} lib the library to look in
  * @param {string[]} scripts the names of all the scripts needed
  * @return {string} the code inside script tags
  */
  ns.libGs = function (lib, scripts) {
    var libOb = eval(lib);
    if(!libOb) {
      throw 'library ' + lib + ' not defined';
    }
    return '<script>\n' + scripts.map (function (d) {
      // getResource returns a blob
      return libOb.showMyScriptAppResource(d).getDataAsString();
    })
    .join('\n\n') + '</script>\n';
  };
